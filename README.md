...
- 대용량 파일 합치기는 파일 올리기와 미리보기 생성 중에도 진행률 창을 표시합니다.
...
엑셀/CSV 파일에서 개인정보로 보이는 컬럼을 자동 인식하고, 사용자가 체크박스로 확정한 컬럼의 값을 가린 새 결과 파일을 만듭니다.

- 이름, 주민등록번호/외국인등록번호, 연락처, 이메일, 계좌번호, 주소, 성별, 나이, 생년월일 컬럼을 컬럼명으로 자동 인식
- 비고 같은 일반 텍스트 컬럼도 값 안의 주민등록번호, 전화번호, 이메일, 생년월일 패턴을 보고 자동 인식
- 자동 인식된 컬럼은 체크박스에 미리 체크되어 표시
- 체크를 해제해 마스킹에서 빼거나, 다른 컬럼을 추가로 체크해 마스킹 대상에 포함 가능
- 마스킹 값은 원본 글자 수만큼 `*`로 표시
...
- 대용량 파일은 파일 올리기와 미리보기 생성 중에도 진행률 창에서 현재 단계를 표시
...
- 후 파일에서 사라진 행은 `후파일_메모` 시트 맨 아래에 전 파일 값으로 추가하고 빨간색 전체 행과 취소선으로 표시
...
8. 대용량 데이터는 파일 올리기, 미리보기 생성, 실행 중에 진행률 창에서 현재 단계와 퍼센트를 확인합니다.
...
| 전/후 파일 검증 | 빨간색 + 취소선 | 후 파일에서 사라진 행입니다. 첫 시트 맨 아래에 전 파일 값으로 추가되고 취소선이 그어집니다. |
