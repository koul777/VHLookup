...
- `8. 여러 시트 합치기`를 추가했습니다. 한 엑셀 파일에서 선택한 시트를 컬럼명 기준으로 아래로 이어 붙이고 `원본 시트명`을 남깁니다.
- 피벗 요약에서 처리할 시트를 복수 선택할 수 있으며, 첫 시트뿐 아니라 선택한 모든 시트를 합쳐 집계합니다.
- 메인 화면, 기능별 전후 이미지, 사용설명서 PDF를 8개 메뉴 기준으로 갱신했습니다.
- 실제 샘플 파일로 실행한 메뉴별 활용 영상 8개와 전체 이어보기 영상을 추가했습니다.
...
각 영상은 실제 VHLookup 화면에서 제공 샘플을 선택하고 결과 엑셀을 생성하는 과정을 보여줍니다. 먼저 전체 흐름을 보려면 [1~8번 메뉴 전체 활용 영상](docs/videos/VHLookup_menu_walkthrough.mp4)을 재생하세요.

| 메뉴 | 활용 영상 |
| --- | --- |
| 1. 개인정보 마스킹 | [영상 보기](docs/videos/01_privacy_masking.mp4) |
| 2. 분류별 시트 나누기 | [영상 보기](docs/videos/02_split_sheets.mp4) |
| 3. 엑셀/CSV 파일 여러 개 합치기 | [영상 보기](docs/videos/03_merge_files.mp4) |
| 4. 전/후 파일 검증 | [영상 보기](docs/videos/04_before_after_validation.mp4) |
| 5. 월별표 목록형 변환 | [영상 보기](docs/videos/05_horizontal_table.mp4) |
| 6. 피벗 요약표 만들기 | [영상 보기](docs/videos/06_pivot_summary.mp4) |
| 7. 사용자 지정 순서 정렬 | [영상 보기](docs/videos/07_organization_order.mp4) |
| 8. 여러 시트 합치기 | [영상 보기](docs/videos/08_sheet_merge.mp4) |
...
한 엑셀 파일에서 선택한 여러 시트를 컬럼명 기준으로 아래로 이어 붙입니다. 결과의 `원본 시트명` 컬럼에서 각 행이 어느 시트에서 왔는지 확인할 수 있습니다.

![여러 시트 합치기 전후 비교](docs/images/feature_08_merge_sheets.png)

## 최근 업데이트

### 2026-08-24

- 한 엑셀 파일 안의 여러 시트를 선택해 `합친결과` 시트로 만드는 기능을 추가했습니다.
- 시트마다 없는 컬럼은 삭제하지 않고 빈칸으로 유지하며, 빈 시트는 `확인사항`에 기록하고 제외합니다.
- 피벗 요약에서 처리할 시트를 복수 선택할 수 있으며, 첫 시트뿐 아니라 선택한 모든 시트를 합쳐 집계합니다.
...
- 행 기준 선택
- 처리할 시트 복수 선택(기본값: 전체 시트)
- 시트 선택을 바꾼 뒤 `선택 시트 불러오기`로 컬럼과 미리보기 갱신
- 열 기준 선택 또는 선택 안 함
- 값 열 선택
...
파일 안의 컬럼을 하나 고른 뒤, 그 컬럼에 들어 있는 실제 값 목록을 원하는 순서로 배치해 전체 행을 정렬합니다.

- 파일을 올리면 컬럼 목록을 자동으로 표시
- 컬럼을 선택하면 해당 컬럼의 고유값을 데이터에서 자동 추출
- 값 목록을 `맨 위`, `위로`, `아래로`, `맨 아래` 버튼으로 재배치
- 부서 직제순뿐 아니라 상태, 등급, 구분처럼 사용자가 원하는 업무 순서에 맞춰 정렬
- 순서 목록에 없는 값은 뒤쪽에 원래 행 순서를 유지해 배치
- 결과 시트: `결과`, `확인사항`

### 8. 여러 시트 합치기

한 엑셀 파일 안에서 같은 구조의 월별·부서별 시트를 하나의 목록으로 합칩니다.

- 합칠 시트를 2개 이상 복수 선택
- 같은 의미의 컬럼명은 한 컬럼으로 정렬하고, 어느 시트에만 있는 컬럼도 보존
- 데이터가 없는 빈 시트는 제외하고 `확인사항`에 기록
- `원본 시트명` 컬럼으로 각 행의 출처 확인
- 결과 시트: `합친결과`, `확인사항`
...
샘플은 프로그램 안에서 새로 만들지 않고 파일로 제공합니다. 폴더는 실행 화면의 1~8번 기능 순서에 맞춰 정리되어 있습니다.
...
- 누를 기능: `6. 피벗 요약표 만들기`
- 선택할 파일: `samples\public_admin\08_sheet_merge\monthly_budget_sheets.xlsx`
- 처리할 시트: `1월`, `2월` 선택 후 `선택 시트 불러오기` (`빈시트`는 자동 제외)
...
- 누를 기능: `7. 사용자 지정 순서 정렬`
- 선택할 파일: `samples\public_admin\07_organization_order\department_tasks.csv`
- 화면 선택값: 컬럼 `부서` 선택 후 값 순서를 `기획조정실`, `총무과`, `인사과`, `예산과`, `복지정책과`, `민원봉사과`, `홍보담당관`, `감사담당관` 순서로 조정
- 결과에서 먼저 볼 시트: `결과`
- 확인 포인트: 선택한 컬럼의 실제 값 목록을 프로그램이 불러오고, 사용자가 움직인 값 순서대로 전체 행이 정렬되는지 확인

**여러 시트 합치기**

- 누를 기능: `8. 여러 시트 합치기`
- 선택할 파일: `samples\public_admin\08_sheet_merge\monthly_budget_sheets.xlsx`
- 합칠 시트: `1월`, `2월`, `빈시트` 선택 (`빈시트`는 데이터가 없어 자동 제외)
- 결과에서 먼저 볼 시트: `합친결과`
- 그 다음 참고 시트: `확인사항`
- 확인 포인트: `1월`, `2월` 데이터가 세로로 이어지고 `원본 시트명`과 `메모` 컬럼이 보존되는지 확인
